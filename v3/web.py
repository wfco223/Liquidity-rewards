...
function sweepCard(sw){
 ...
 if(window._sweepNote){h+='<div class="sub ok">'+esc(window._sweepNote)+'</div>';}
 if(sw.error){h+='<div class="warn">The last sweep stopped with an error: '+esc(sw.error)+'</div>';}
 if(busy){h+='<div class="sub warn"><b>'+(busy.op==='run'?'Placing':'Previewing')+'\\u2026</b> '+esc(busy.phase||'')+' '+(busy.done||0)+' of '+(busy.total||0)+' <span class="muted">(this card refreshes itself every few seconds)</span></div>';}
 if(last.at){h+='<div class="sub">Last run '+when(last.at)+': placed <b>'+(last.n||0)+'</b> exits worth '+usd(last.value||0)+(last.filled?' \\u00b7 <b>'+last.filled+'</b> shares went at once':'')+(last.unsaid?' \\u00b7 '+last.unsaid+' the exchange did not report on \\u2014 their fills book on the next cycle':'')+(last.resting?' \\u00b7 '+last.resting+' left resting':'')+(last.hand?' (replaced '+last.hand+' of your hand orders)':'')+((last.failed||[]).length?' \\u00b7 <span class="warn">'+last.failed.length+' refused</span>':'')+((last.skipped||[]).length?' \\u00b7 '+last.skipped.length+' skipped':'')+'</div>';
  if((last.placed||[]).length){h+='<details class="how"><summary class="muted">what it crossed</summary>'+last.placed.map(function(r){return '<div class="muted">'+esc(r.name||r.market)+': '+(r.qty>0?'sold':'bought back')+' '+Math.abs(r.qty)+' at '+pc(r.price)+' (worth '+usd(r.value)+')'+(r.filled?' \\u00b7 '+r.filled+' filled':'')+(r.unsaid?' \\u00b7 no execution list in the answer':'')+(r.resting?' \\u00b7 '+r.resting+' resting':'')+((r.replaced||[]).length?' \\u00b7 replaced '+r.replaced.length:'')+'</div>';}).join('')+'</details>';}
 ...
 if(!busy&&pv.at&&pv.rows){h+='<div class="sub"><b>Preview</b> ('+when(pv.at)+'): '+pv.n+' holdings worth '+usd(pv.value||0)+'; crossing gives up <b>'+usd(pv.concession||0)+'</b> against the midpoint; '+pv.replace+' orders replaced ('+pv.hand+' yours); '+(pv.skipped||[]).length+' skipped.</div>';
  h+='<details class="how" open><summary class="muted">what it would cross</summary>'+pv.rows.map(function(r){return '<div class="muted">'+esc(r.name||r.market)+': '+(r.qty>0?'sell':'buy back')+' '+Math.abs(r.qty)+' at '+pc(r.price)+' (bid '+pc(r.bid)+' / ask '+pc(r.ask)+', worth '+usd(r.value)+', gives up '+usd(r.concession||0)+')'+((r.replace||[]).length?' \\u00b7 replaces '+r.replace.length:'')+'</div>';}).join('')+'</details>';
  ...
  h+='<div><button onclick="sweepOp(\\'run\\')">Cross '+pv.n+' lots</button> <button class="off" onclick="sweepOp(\\'preview\\')">Refresh preview</button></div>';}
 else if(!busy){h+='<div><button onclick="sweepOp(\\'preview\\')">Preview the sweep</button></div><div class="hint">Preview reads the books (it can take a minute or two while the exchange is throttling) and shows every order it would place. Nothing is placed until you tap Place.</div>';}
 return h+'</div>';
}
function sweepPoll(){fetch('/sweep.json',{headers:hdrs(),cache:'no-store'}).then(function(r){if(r.status===401)return null;return r.json();}).then(function(j){if(!j)return;var was=JSON.stringify(window._sweep||{});window._sweep=j;if(JSON.stringify(j)!==was&&window._d)render(window._d);}).catch(function(){});}
function sweepOp(op){if(op==='run'&&!confirm('Sell these lots across the spread now? They CROSS — a long sells at the bid, a short buys back at the ask — so they trade at once rather than resting. Every order on those sides comes off first, your hand orders included (not your walls).'))return;window._sweepNote='';post({op:'sweep_'+op},function(j){window._sweepNote=j.note||'';sweepPoll();});}
sweepPoll();setInterval(sweepPoll,4000);
...
function vCard(v){if(!v)return '';if(!v.ok)return '<div class="card muted">stage 2: '+esc(v.note||'not planned yet')+'</div>';
 var o='<div class="card"><b>What $'+Math.round(v.pot).toLocaleString()+' would earn</b> <span class="pill">stage 2 — read only</span>'
  +'<div class="muted">For every side of every tier market: the reward our order would claim (our share of the side’s pool, by the exchange’s own arithmetic), less the fills it would take (fills a day, measured on paper orders, × shares × what a fill lost an hour later, never under '+pc(v.fill_floor)+' a share), less the money it ties up ('+(v.coc_day*100).toFixed(1)+'% a day). Our own resting orders are taken out of the books; yours stay in as company. Money goes out $'+v.slice+' at a time to whichever spot adds the most a dollar'+(v.market_cap>=v.pot?'; no cap per market — each stops where its next $'+v.slice+' earns less than its fills and capital cost (your call, 09-26)':'; no market takes more than $'+Math.round(v.market_cap))+'. Nothing here places an order.</div>';
...
// Stage 3 (owner, 2026-09-26 "We need to go back to building the bigger
// 4 tier earning machine"): an engine per tier trading on paper. READ-ONLY.
...
function pTier(p,key){if(!p||!p.ok)return '';var t=(p.tiers||{})[key];if(!t)return '';var d=t.today||{};
 ...
 var sh=pShadow(t.shadow);if(sh)o+='<div class="sub muted">'+sh+'</div>';
 return o;}
function pList(p,key){if(!p||!p.ok)return '';var t=(p.tiers||{})[key];if(!t)return '';var o='';
...
var F_SORTS=[['ev','entry EV'],['name','name'],['rate','earning'],['shares','holding'],['drop','drop from peak'],['unq','unqualified'],['boost','wall boost']];
var F_SORT_FN={name:function(a,b){return String(a.name||a.market).localeCompare(String(b.name||b.market));},
 ...
 boost:function(a,b){return ((b.boost_day||0)-(a.boost_day||0))||((b.unqualified||0)-(a.unqualified||0))||((b.rate||0)-(a.rate||0));}};
...
function fHead(f){
 ...
 +' → entry stake <b>'+usd(f.stake)+'</b>'+(f.stake_bp!=null&&f.stake_src!=='set by you'?' <span class="muted">('+esc(f.stake_src)+': '+usd(f.stake_bp)+' = the exchange\'s buying power less the '+usd(f.keep_free||0)+' kept free'+(f.walls_held>0.5?'; your walls hold '+usd(f.walls_held)+' on top of that, already counted out by the exchange':'')+')</span>':' ('+esc(f.stake_src)+')')+' · pass '+f.pass_s+'s, '+fBooks(f)+(f.at?' · '+when(f.at):'')+'</div>'
...
function fWallBtns(r){var w=r.wall;if(!w)return '';var o='';var q=r.qual||{};
 ['ask','bid'].forEach(function(s){var x=w[s];if(!x||x.room)return;var ok=q[s]&&q[s][2];
  ...
  var b=(r.boost||{})[s];
  if(b){o+='<div class="muted" style="font-size:12px;margin:2px 0 6px">'+(b.usd_day>0?'<span class="ok">+'+usd(b.usd_day)+'/day</span> once the '+s+' side qualifies — '+esc((b.parts||[]).join('; ')):esc(b.note||'no estimate'))+' <span class="muted">('+fmtsz(b.gap||0)+' shares at '+pc(b.px)+', ~'+usd(b.usd||0)+' of buying power held)</span></div>';}
  else if(!ok){o+='<div class="muted" style="font-size:12px;margin:2px 0 6px">no boost estimate for the '+s+' side yet</div>';}});
...
function fRow(r,f){
 ...
 if(r.boost_day>0)pills.push('<span class="pill on">wall: +'+usd(r.boost_day)+'/day</span>');
...
function fDetail(r,f){
 ...
 if(r.position){var ex=null;od.forEach(function(x){if(x.exit&&!ex)ex=x;});
  o.push('<div class="sub"><b>'+(r.position.qty>0?'+':'')+r.position.qty+' held @ '+pc(r.position.cost_px)+'</b>'+(r.position.est?' <span class="muted">(cost estimated by the exchange\'s feed — no loss is paged or learned from it)</span>':'')+(ex?' · exit resting '+ex.qty+' @ '+pc(ex.price)+' → ~'+usd(ex.est||0)+'/day':(r.exit&&r.exit.px!=null?' · <span class="warn">exit planned '+r.exit.qty+' @ '+pc(r.exit.px)+', not resting yet</span>':(r.exit&&r.exit.note?' · <span class="warn">'+esc(r.exit.note)+'</span>':(r.fair==null?' · <span class="warn">no exit tended — set a fair or rest one yourself</span>':''))))+'</div>');}
 ...
 o.push('<div class="sub">stake a side <span class="muted">'+usd(r.stake)+' ('+esc(r.stake_src)+')</span>'+(r.stake_top>r.stake+1?' <span class="muted">· a top-quartile entry may grow to '+usd(r.stake_top)+(r.top_cut!=null?', clearing '+usd(r.top_cut)+'/day per $ of expected loss at every slice':', once there are enough readings to call a quartile')+'</span>':'')+' '+fField('fs-'+r.market,'','5em','$')+fBtn('Set','fStake(\''+m+'\')','small')+(r.stake_src==='set by you'?fBtn('Back to 10%','fOp(\'focus_stake\',\''+m+'\',\'\')','small off'):'')+'</div>');
...
function fMath(r,f){
 ...
 ['BUY','SELL'].forEach(function(sd){var x=td[sd];if(!x)return;var lab=(sd==='BUY'?'bid':'ask');
  ...
  o.push('<div class="muted">tender '+lab+(x.exit?' (exit)':'')+': '+x.qty+' @ '+pc(x.px)+(x.exit&&x.basis!=null?' · cost '+pc(x.basis):'')+' → ~'+usd(x.est)+'/day, odds '+Math.round(x.pf*100)+'%'+(!x.exit?', EV '+fSign(x.ev)+'/day':'')+(x.scale_note?' · '+esc(x.scale_note):'')+(x.grow_note?' · <b>'+esc(x.grow_note)+'</b>':'')+'</div>');});
            ...
            def do_GET(self):  # noqa: N802
                ...
                if route == "/sweep.json":
                    if not authed(self.headers.get, u.query, server.password):
                        self._send(401, "application/json", b'{"error":"key required"}')
                        return
                    self._send(200, "application/json", server.monitor.sweep_json())
                    return
                if route == "/fills.json":
                    if not authed(self.headers.get, u.query, server.password):
                        self._send(401, "application/json", b'{"error":"key required"}')
