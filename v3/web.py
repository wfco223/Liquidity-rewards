...
function sweepCard(sw){
 ...
 if(last.at){h+='<div class="sub">Last run '+when(last.at)+': placed <b>'+(last.n||0)+'</b> exits worth '+usd(last.value||0)+(last.filled?' \\u00b7 <b>'+last.filled+'</b> shares went at once':'')+(last.resting?' \\u00b7 '+last.resting+' left resting':'')+(last.hand?' (replaced '+last.hand+' of your hand orders)':'')+((last.failed||[]).length?' \\u00b7 <span class="warn">'+last.failed.length+' refused</span>':'')+((last.skipped||[]).length?' \\u00b7 '+last.skipped.length+' skipped':'')+'</div>';
  if((last.placed||[]).length){h+='<details class="how"><summary class="muted">what it crossed</summary>'+last.placed.map(function(r){return '<div class="muted">'+esc(r.name||r.market)+': '+(r.qty>0?'sold':'bought back')+' '+Math.abs(r.qty)+' at '+pc(r.price)+' (worth '+usd(r.value)+')'+(r.filled?' \\u00b7 '+r.filled+' filled':'')+(r.resting?' \\u00b7 '+r.resting+' resting':'')+((r.replaced||[]).length?' \\u00b7 replaced '+r.replaced.length:'')+'</div>';}).join('')+'</details>';}
