...
function bAmp(r){
 ...
 if(a.qty!=null)return ' · <span class="pill on">amplifier</span> '+a.qty+' @ '+pc(bTerms(r,a.px))+' a tick behind the exit — our size at that level claims more of the side → adds ~'+usd(a.gain||0)+'/day, expected cost '+usd(a.exp_loss||0)+'/day';
...
function bList(d){
 ...
 if(b.amp_budget!=null)out+='<div class="sub">Amplifiers: a buy of the underdog a tick behind each exit, so our size at the exit\u2019s level claims more of the side\u2019s reward. Sized where the earnings it adds pay for its expected loss at least twice over; all together within '+usd(b.amp_budget)+'/day of expected loss ('+Math.round((b.amp_fraction||0.5)*100)+'% of the bonds\u2019 '+usd(b.amp_avg_earn||0)+'/day average over the last few days). '+usd(b.amp_in_play||0)+'/day in play, adding ~'+usd(b.amp_unlocks||0)+'/day. An exit fill pulls them for two hours; the exit sits ahead of them in the book.</div>';
