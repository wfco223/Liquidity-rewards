...
function render(d){
 ...
 if(b.money_out){var mo=b.money_out;out+='<div class="sub warn"><b>No money to deploy</b> since '+when(mo.since)+' ('+usd(mo.bp||0)+' free on the exchange'+(mo.walls>0?', the '+usd(mo.walls)+' the walls hold not counted against you':'')+'): the bonds only sell, never below cost. '+(mo.pulled||0)+' buy-more order'+(mo.pulled===1?'':'s')+', every decoy and bait came off ('+usd(mo.freed||0)+' freed). Nothing is bought, sniped or baited until '+usd((b.money_back_usd||50)+(mo.freed||0))+' is free.</div>';}
